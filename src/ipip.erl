...
init(FileDAT) ->
	...
	FormatTime = fun({{Ye, Mo, Da}, {Ho, Mi, Se}}) -> io_lib:format("~p-~p-~p ~p:~p:~p~n", [Ye, Mo, Da, Ho, Mi, Se]) end,
	io:put_chars(IoDevice, "%%% 		atime: " ++ FormatTime(Atime)),
	io:put_chars(IoDevice, "%%% 		ctime: " ++ FormatTime(Ctime)),
	io:put_chars(IoDevice, "%%% 		mtime: " ++ FormatTime(Mtime)),
